...
tldr: 离散扩散模型微调面临奖励不可微且仅利用终端奖励的局限。本文提出连续时间强化学习框架，将状态建模为受控连续时间马尔可夫链，推导出连续时间PPO和GRPO算法。该框架支持沿去噪轨迹的中间奖励信号，并针对掩码扩散模型提出轨迹子采样以降低计算成本。在低维优化和dLLM数学推理与编码任务上验证有效性，为离散扩散模型RL后训练提供了统一且高效的新视角。
source: arxiv
...
motivation: 现有离散扩散模型微调依赖可微奖励且仅利用终端奖励，无法利用中间过程信息进行优化，限制了性能提升和算法泛化能力。
method: 提出连续时间RL框架，用受控CTMC建模状态，推导连续时间PPO和GRPO，在掩码扩散模型中实现解析概率比率并引入轨迹子采样降低计算量。
result: 在低维熵正则化优化和dLLM数学推理与编码任务上，该方法有效提升性能且计算成本可控，优于仅用终端奖励的基线。
conclusion: 该框架无需奖励可微，支持中间反馈，为离散扩散模型提供了统一的探索与策略优化视角，拓展了RL后训练的适用范围。
---

## 摘要
我们通过随机控制方法，在离散状态空间和可能任意动作空间的连续时间内构建了强化学习框架，其中状态动态被建模为受控的连续时间马尔可夫链。我们考虑策略优化问题，并推导了相应的策略梯度方法，从而得到了近端策略优化和组相对策略优化的连续时间变体。作为主要应用，我们开发了一个完整的连续时间强化学习框架，用于微调基于得分的离散扩散模型。该框架实现了奖励驱动的优化，无需对奖励信号可微。与仅依赖终端奖励的现有基于GRPO的方法不同，我们的公式允许在去噪轨迹中纳入中间奖励或优势信号。重要的是，当专门应用于掩码扩散模型时，我们的框架在词汇单纯形上涵盖了丰富的策略参数化类，并具有解析可处理的概率比，为掩码扩散模型中的探索和策略优化提供了统一视角。对于掩码扩散大语言模型，我们进一步提出了轨迹子采样技术，以高效估计计算上难以处理的轨迹似然，降低了计算每个位置概率比的计算成本。我们在低维熵正则化优化问题以及数学推理和编码任务上的dLLMs强化学习后训练中展示了我们方法的有效性。
