...
tldr: 模型预测控制（MPC）在工业与机器人系统中广泛应用，但无法为带有未来信息的马尔可夫决策过程（MDP）提供最优策略。现有MPC-强化学习（RL）方法未考虑未来信息导致的增强状态结构。本文建立了参数化MPC精确表示带未来信息MDP最优价值函数和策略所需的结构性条件，并利用RL学习其参数。理论证明了参数化MPC可作为结构化函数逼近器，在点质量赛车任务上验证了方法的有效性，为MPC-RL结合处理动态信息提供了新途径。
source: arxiv
...
motivation: 现有MPC-RL方法无法处理包含未来信息（如预测、价格）的MDP，需要将未来信息纳入状态以实现最优决策。
method: 建立参数化MPC精确表示带未来信息MDP最优价值函数和策略的结构性要求，并使用RL学习其参数。
result: 证明参数化MPC可作为结构化函数逼近器，在点质量赛车任务上验证了策略优化的有效性。
conclusion: 表明参数化MPC能有效解决带未来信息的MDP，扩展了MPC-RL框架的应用范围。
---

## 摘要
模型预测控制（MPC）在工业和机器人系统中被广泛用于通过基于有限时域优化的规划来强制执行约束和嵌入领域知识。然而，尽管有这些优势，MPC方案通常无法为表示为马尔可夫决策过程（MDP）的序贯决策问题产生最优策略。最近MPC与强化学习（RL）的结合通过将MPC视为MDP最优策略的参数化模型，并使用数据调整其参数来缓解这一问题。虽然这些方法通常考虑经典的MDP，但许多实际问题在决策时包含未来信息——例如预测、价格或参考轨迹——这些信息必须包含在MDP状态中才能实现最优决策。当前的MPC-RL方法没有直接考虑这种增广状态结构，这引发了如何将未来信息纳入MPC以获得最优策略的问题。本文建立了参数化MPC能够精确表示具有未来信息的MDP的最优值函数和策略的结构性要求。我们进一步证明，这种参数化MPC可以作为结构化函数逼近器，其参数通过RL学习。该方法在一个带有未来参考信息的质点竞速任务上进行了说明。
